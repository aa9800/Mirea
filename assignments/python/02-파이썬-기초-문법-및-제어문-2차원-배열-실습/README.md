...
파이썬의 기본 사칙연산, 변수 선언, 문자열 출력 및 사용자 입력을 다루는 실습 코드입니다. 입력받은 데이터의 타입을 정수로 변환하여 섭씨-화씨 온도 변환과 만기 원리금을 계산하는 기초적인 수식 프로그램을 구현합니다.
...
사용자로부터 숫자를 입력받아 정수형으로 변환하는 기본 예제와 온도 변환, 원리금 계산 로직을 다루는 코드입니다. 변수 타입 확인(type)과 문자열 분할(split)을 통한 다중 입력 처리 방법을 복습합니다.
...
조건문, while/for 반복문, 리스트 조작, 로또 번호 생성, 369 게임, 소수 판별 등 파이썬 기초 알고리즘 예제를 다룹니다. 중첩 반복문을 활용한 피라미드 형태의 별찍기와 리스트 교집합 구하기 등 다채로운 흐름 제어를 연습합니다.
...
비밀번호 로그인 시도 횟수 제한 로직과 종료 메뉴가 포함된 자판기 프로그램을 구현한 파일입니다. while 무한 루프와 break, continue 제어문을 통해 대화형 프로그램을 제어하는 방법을 실습합니다.
...
홀수/짝수 인덱스에 따라 서로 다른 기호를 출력하는 반복문과 2차원 리스트 기반 좌석 예약 시스템을 구현했습니다. 사용자로부터 행과 열 번호를 입력받아 좌석 상태를 예약(1)으로 업데이트하고 현황을 출력하는 방식을 다룹니다.
