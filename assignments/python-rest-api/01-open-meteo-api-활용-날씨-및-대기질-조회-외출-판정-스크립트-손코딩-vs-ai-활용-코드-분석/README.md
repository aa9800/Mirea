# Open-Meteo API 활용 날씨 및 대기질 조회 외출 판정 스크립트, 손코딩 vs ai 활용 코드 분석

- 과목: Python / REST API
- 날짜: 2026-08-19
- 태그: python, requests, open-meteo, api, multithreading

## 설명

파일 구조: weather.py, air.py는 따로 만들어졌으나, app.py는 이 둘을 함수로 나눠서 한 파일에 다 넣음
도시 이름 검색: 손코딩 쪽은 results[0]만 꺼내 써서 검색 결과 없으면 에러 발생, 한글로 "서울" 입력 시 검색도 안 됨. app.py는 한글-영문 매핑 딕셔너리를 만들어서 변환 후 검색하고, 결과 없어도 안내 메시지만 띄우고 종료되지 않음
에러 처리: 손코딩 쪽은 timeout=5만 걸려 있어서 응답 이상하거나 타임아웃 나면 프로그램 종료됨. app.py는 try/except로 감싸서 실패한 항목만 "정보 없음"으로 처리하고 나머지는 계속 진행함
강수확률 값: weather.py는 hourly 배열의 [0]번째 값을 그대로 사용. app.py는 현재 시각과 시간대를 맞춰서 정확한 값을 찾아 사용
API 호출 방식: 손코딩 쪽은 날씨, 대기질 API를 순서대로 하나씩 호출. app.py는 스레드로 두 API를 동시에 호출
판정 로직: air.py는 미세먼지 등급(GOOD/CAUTION/BAD)만 나오고 끝. app.py는 기온, 강수확률, 미세먼지를 각각 점수로 매겨서 합산한 다음 외출 가능/주의/비추천까지 판정
입력 방식: 손코딩 쪽은 sys.argv[1]로만 도시명을 받음. app.py는 인자가 없으면 직접 입력받게 처리했고, 윈도우에서 한글 깨지는 것도 방지함
...
<details>
<summary><strong>air.py</strong> — Open-Meteo 지오코딩 API와 대기질 API를 연동하여 특정 도시의 초미세먼지(PM2.5) 및 미세먼지(PM10) 농도를 조회하는 스크립트입니다. 수집한 PM2.5 수치에 따라 GOOD, CAUTION, BAD 3단계 등급을 산출하여 결과를 출력합니다.</summary>
...
<details>
<summary><strong>app.py</strong> — 날씨 API와 대기질 API를 ThreadPoolExecutor로 동시 호출하여 기온, 강수확률, PM2.5 정보를 종합적으로 분석하고 외출 권장 여부를 한 줄로 판정하는 통합 실행 파일입니다. 한글 도시명 영문 매핑 테이블과 타임아웃 및 실패 대응 로직이 포함되어 있어 안정적인 실행을 보장합니다.</summary>
...
<details>
<summary><strong>first_call.py</strong> — Open-Meteo 날씨 예측 API에 기본 GET 요청을 보내고 HTTP 상태 코드 및 응답 본문의 일부를 확인하는 기초 연동 테스트 스크립트입니다. API 호출 정상 작동 여부 및 기본 응답 형태를 검증하는 용도로 작성되었습니다.</summary>
...
<details>
<summary><strong>show_json.py</strong> — API 응답 데이터를 파이썬 딕셔너리로 변환하고 주요 키 목록과 현재 기온 데이터를 추출하는 가공 연습용 스크립트입니다. json.dumps 기법을 사용하여 딕셔너리 구조를 들여쓰기된 보기 좋은 문자열 형태로 출력합니다.</summary>
...
<details>
<summary><strong>weather.py</strong> — 입력받은 도시 이름을 지오코딩 API로 위도와 경도로 변환한 후 해당 좌표의 현재 기온과 시간별 강수확률을 조회하는 스크립트입니다. 실행 시 인수로 도시명을 전달받거나 기본값(Seoul)을 사용하여 날씨 정보를 출력합니다.</summary>
...
- [air.py](./code/1787120734217-19224917.py)
- [app.py](./code/1787120734218-385204807.py)
- [first_call.py](./code/1787120734219-101648508.py)
- [show_json.py](./code/1787120734220-469106555.py)
- [weather.py](./code/1787120734221-350400952.py)

## 실행 결과

```
[위치] Seoul, South Korea  (위도 37.5683, 경도 126.9778)
[기온] 18.5℃ (쾌적)
[강수확률] 10% (낮음)
[PM2.5] 12.0µg/㎥ (좋음)
...
requests 라이브러리를 사용한 REST API HTTP GET 요청 및 JSON 데이터 파싱 방법과, ThreadPoolExecutor를 활용한 멀티스레드 동시 호출 및 타임아웃 예외 처리 기법을 배울 수 있습니다.

## 어려웠던 점

외부 API 호출 시 응답 지연이나 네트워크 실패 상황에 대비하여 각 요청별 타임아웃을 설정하고, 멀티스레드 환경에서 한쪽 요청이 실패하더라도 잔여 정보로 안정적으로 판정을 이어가도록 예외 처리를 구성하는 부분이 까다로울 수 있습니다.

---
_Study Archive에서 자동 생성됨 · 마지막 수정: 2026-08-19T06:25:34.229Z_